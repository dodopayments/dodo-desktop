...
- Environment details (OS, Rust/Node versions, architecture)
...
Because the desktop app auto-updates from GitHub Releases, we strongly encourage all users to stay on the latest version.

### Scope

This policy covers vulnerabilities in this repository's source code and release artifacts (installers, updater bundles, code signatures). Vulnerabilities in third-party dependencies should be reported upstream; however, if a dependency issue critically impacts this project, you may still open an advisory here and we will coordinate as needed.

Vulnerabilities in the underlying Dodo Payments web application (rendered inside this desktop wrapper) should be reported via the main [Dodo Payments security policy](https://github.com/dodopayments/billingsdk/security).
